...
if not os.path.exists('../../data/004_OCR_correction_evaluation'):
    os.makedirs('../../data/004_OCR_correction_evaluation')

for year in years:

    if not os.path.exists(os.path.join('../../data/004_OCR_correction_evaluation', year)):
        os.makedirs(os.path.join('../../data/004_OCR_correction_evaluation', year))

    file_list = [tmp for tmp in os.listdir(os.path.join(datapath_annotation, year)) if not tmp.endswith('.txt')]
    for file_name in file_list:

        if not os.path.exists(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name)):
            os.makedirs(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name))
        ...
        for table in tables:
            ...
            os.makedirs(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table),
                        exist_ok=True)
            os.makedirs(
                os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table, 'correction'),
                exist_ok=True)
            ...
            # store aligned data
            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'aligned_frak2021_data.txt'), 'w') as f:
                f.write("\n".join(aligned_ocr_data[0]))
            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'aligned_Fraktur_data.txt'), 'w') as f:
                f.write("\n".join(aligned_ocr_data[1]))
            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'aligned_GT4HistOCR_data.txt'), 'w') as f:
                f.write("\n".join(aligned_ocr_data[2]))

            # prepare data for all input orders
            ocr_pairs = [[], [], [], [], [], []]
            ...
            for line1, line2, line3 in zip(aligned_ocr_data[0], aligned_ocr_data[1], aligned_ocr_data[2]):
                ...
                line2 = line2.replace('\n', '')
                line3 = line3.replace('\n', '')
                ocr_pairs[0].append(f"({line1};{line2};{line3})")

            for line1, line2, line3 in zip(aligned_ocr_data[0], aligned_ocr_data[2], aligned_ocr_data[1]):
                line1 = line1.replace('\n', '')
                line2 = line2.replace('\n', '')
                line3 = line3.replace('\n', '')
                ocr_pairs[1].append(f"({line1};{line2};{line3})")

            for line1, line2, line3 in zip(aligned_ocr_data[1], aligned_ocr_data[0], aligned_ocr_data[2]):
                line1 = line1.replace('\n', '')
                line2 = line2.replace('\n', '')
                line3 = line3.replace('\n', '')
                ocr_pairs[2].append(f"({line1};{line2};{line3})")

            for line1, line2, line3 in zip(aligned_ocr_data[1], aligned_ocr_data[2], aligned_ocr_data[0]):
                line1 = line1.replace('\n', '')
                line2 = line2.replace('\n', '')
                line3 = line3.replace('\n', '')
                ocr_pairs[3].append(f"({line1};{line2};{line3})")

            for line1, line2, line3 in zip(aligned_ocr_data[2], aligned_ocr_data[1], aligned_ocr_data[0]):
                line1 = line1.replace('\n', '')
                line2 = line2.replace('\n', '')
                line3 = line3.replace('\n', '')
                ocr_pairs[4].append(f"({line1};{line2};{line3})")

            for line1, line2, line3 in zip(aligned_ocr_data[2], aligned_ocr_data[0], aligned_ocr_data[1]):
                line1 = line1.replace('\n', '')
                line2 = line2.replace('\n', '')
                line3 = line3.replace('\n', '')
                ocr_pairs[5].append(f"({line1};{line2};{line3})")

            response = [[], [], [], [], [], []]
            for idx in range(len(ocr_pairs)):
                print('start correcting: ' + year + ' ' + table + ' ' + 'order: ' + str(order[idx]))
                chunk_size = 1
                chunk_list = [np.array(ocr_pairs[idx])[i:i + chunk_size] for i in
                              range(0, len(np.array(ocr_pairs[idx])), chunk_size)]
                result = ''
            ...
            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'correction', 'ocr_correct_123.txt'), 'w') as f:
                f.write(response[0])

            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'correction', 'ocr_correct_132.txt'), 'w') as f:
                f.write(response[1])

            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'correction', 'ocr_correct_213.txt'), 'w') as f:
                f.write(response[2])

            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'correction', 'ocr_correct_231.txt'), 'w') as f:
                f.write(response[3])

            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'correction', 'ocr_correct_321.txt'), 'w') as f:
                f.write(response[4])

            with open(os.path.join('../../data/004_OCR_correction_evaluation', year, file_name, 'ocr_correction_' + table.strip('.jpg'),
                                   'correction', 'ocr_correct_312.txt'), 'w') as f:
                f.write(response[5])